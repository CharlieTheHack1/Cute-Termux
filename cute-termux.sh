...
#cd $HOME
#cd Cute-Termux
echo Script made by:- Charlie : The Hacker
#Uninstall
if [ -e ".user.cfg" ]
then
...
updatedw() {
	...
	then
		...
		if [[ \$var == *\"up to date\"* ]];
		then 
			clear && echo \"Already running the latest version!!\" && echo ------------------------------------- && figlet \$(sed '1q;d' 'ver.cfg') && figlet Charlie
		else
			git pull
			bash setup.sh;
			clear && echo \"Update Success\" && echo -------------- && figlet Success && figlet \$(sed '1q;d' 'ver.cfg') && figlet Charlie && echo Restart to apply changes
		fi
	else
		git clone https://github.com/CharlieTheHack1/Cute-Termux.git
		cd Cute-Termux
		prm sh
		clear
		bash
		bash setup.sh
		wait
		echo \"Update Success\" && echo -------------- && figlet Success && figlet \$(sed '1q;d' '/data/data/com.termux/files/home/Cute-Termux/ver.cfg') && figlet Charlie && echo Restart to apply changes
	fi
	cd \$ppath
}
prm() { chmod +x *.\$1; }
...
if [ -d \"\$HOME/Cute-Termux\" ]
then
	if grep -q '# 011' \"/data/data/com.termux/files/home/Cute-Termux/.user.cfg\"
	then
		lnum=\$( sed '3q;d' \"/data/data/com.termux/files/home/Cute-Termux/.user.cfg\" )
		lnum=\$( echo \$lnum | sed 's/# 011//g' )
		lnum=\$( echo \$lnum | sed 's/ //g' )
		echo \$( sed '3q;d' \"/data/data/com.termux/files/home/Cute-Termux/.user.cfg\" )
			if [[ ! \$lnum -eq 5 ]]
			then
				lnum=\$((\$lnum+1))
				sed -i \"/.*# 011.*/ c\\ \$lnum # 011\" \"/data/data/com.termux/files/home/Cute-Termux/.user.cfg\"
			else
				lnum=1
				sed -i \"/.*# 011.*/ c\\ \$lnum # 011\" \"/data/data/com.termux/files/home/Cute-Termux/.user.cfg\"
				cd \$HOME
		...
		echo hi
		echo \"1 # 011\" >> \"/data/data/com.termux/files/home/Cute-Termux/.user.cfg\"
	fi
else
	updatedw
fi
